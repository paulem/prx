...
- A passphrase-protected key must already be in ssh-agent, since batch mode never prompts. ssh reports a locked key and an unauthorized one alike, as `Permission denied (publickey)`, so prx reads the identity file and asks ssh-agent itself to tell them apart: `prx init` says so while the key is being picked, and a tunnel failure names the `ssh-add` command rather than the server.

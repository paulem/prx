...
import { probeUntilLive } from "../probe.ts";
...
export async function runUp({ system, reporter, probeTimeoutMs }: UpCommand): Promise<number> {
  const config = await readConfig(system);
  const proxy = requireBuiltIn(config, "start");
  const report = await bringUp(system, proxy, probeTimeoutMs);
...
/** Starts the proxy unless it is running, then waits for both endpoints up to the probe timeout */
export async function bringUp(
  ...
  const reports = await probeEndpoints(proxy, probeTimeoutMs, probeUntilLive);
